...
public final class ScyllaQueryRunner
{
    ...
    {
        private final TestingScyllaServer scyllaServer;
        private final Map<String, String> connectorProperties = new HashMap<>();
        private List<TpchTable<?>> initialTables = ImmutableList.of();

        private Builder(TestingScyllaServer scyllaServer)
        {
            super(testSessionBuilder()
                    .setCatalog("cassandra")
                    .setSchema("tpch")
                    .build());
            this.scyllaServer = requireNonNull(scyllaServer, "server is null");
        }
                ...
                throws Exception
        {
            ...
            try {
                ...
                createKeyspace(scyllaServer.getSession(), "tpch");
                copyTpchTables(queryRunner, "tpch", TINY_SCHEMA_NAME, initialTables);
                for (TpchTable<?> table : initialTables) {
                    scyllaServer.refreshSizeEstimates("tpch", table.getTableName());
                }
                return queryRunner;
            }
            catch (Throwable e) {
